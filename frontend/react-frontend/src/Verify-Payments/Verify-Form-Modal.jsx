...
export default function VerifyFormModal({
  verifiesInfo,
  isOpen,
  closeModal,
  programCodes,
  ...
  const modal = (
    <div className="verify-modal">
      ...
      <p className="form-title">
        ...
        {verifiesInfo.payments_count}
      </p>
      <p className="form-title">
        <b>Total Amount: </b>&#8369; {verifiesInfo.total_value}
      </p>
      ...
      <div className="form-informations">
        <div className="form-information">
          ...
          <input
            ...
            id="block_rep_name"
            className="form-info"
            value={selectedBlockRep}
            onChange={(e) => setSelectedBlockRep(e.target.value)}
          />
          <datalist id="blockReps">
            {blockReps.map((rep, index) => {
              const detail = `${rep.year_level} - ${rep.program_code}`;
              return (
                <option key={index} value={rep.full_name} title={detail}>
                  {rep.full_name}
                </option>
              );
            })}
                ...
                <tr key={index}>
                  ...
                  <td className="modal-table-cell">
                    <input
                      type="text"
                      name="student_name"
                      value={paymentsInfo.student_name}
                      className="form-control"
                      readOnly
                    />
                  </td>
