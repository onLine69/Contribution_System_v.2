  ...
  const contributionSelector = (
    <div id="contribution-info" className="card">
      ...
      <select
        ...
        id="selected-contribution"
        className="filter-selection"
        onChange={(e) => {
          choosenContribution.fetchPayments(e.target.value, setters);
        }}
      >
        {contributions.map((contribution, index) => {
          return (
            <option key={index} value={contribution.name}>
              {contribution.name}
            </option>
          );
        })}
      ...
      <div className="stat-block">
        ...
        <div className="stat-block-info" style={{ color: "blue" }}>
          {choosenContribution.selectedContribution.name}
        </div>
      </div>
      <div className="stat-block">
        <div className="stat-block-label">Amount:</div>
        <div className="stat-block-info" style={{ color: "green" }}>
          &#8369; {choosenContribution.selectedContribution.amount}
        </div>
      </div>
      <div className="stat-block">
        <div className="stat-block-label">Academic Year:</div>
        <div className="stat-block-info" style={{ color: "red" }}>
          {choosenContribution.selectedContribution.academic_year}
        </div>
      </div>
    </div>
  );
      ...
      {filteredStatus !== "All" ? (
        ...
        <>
          <div className="stat-block">
            ...
            <div className="stat-block-info" style={{ color: "blue" }}>
              &#8369;{" "}
              {stat.paid * choosenContribution.selectedContribution.amount}
            </div>
          </div>
          <div className="stat-block">
          ...
          <div className="stat-block">
            <div className="stat-block-label">Shown Students:</div>
            <div className="stat-block-info" style={{ color: "black" }}>
              {stat.payment_records_length}
            </div>
          </div>
        </>
      )}
